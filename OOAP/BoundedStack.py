...
Сначала подумал, что может возникнуть потребность менять вместимость стека, но потом пришел к выводу, что это 
тогда это будет системная проблема, а увеличение вместимости на ходу будет скорее костылем. 
Раз уж мы ограничиваем стек, то для выполнения конкретной задачи лимита должно хватить.
...
abstract class BoundedStack<T>
    ...
    public const int PUSH_ERR = 2; // стек заполнен
