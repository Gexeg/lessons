'''
tail не сводима к другим операциям,

В текущем описании операция find() перемещает курсор на следующий узел с искомым значением. Чтобы найти все узлы нужно просто вызывать её до 2 статуса.
В первоначальной реализации обход списка начинался заново и требовался отдельный запрос, собирающий все узлы за 1 пробег.
